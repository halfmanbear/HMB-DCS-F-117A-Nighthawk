...
local IRADS_ZOOM = get_param_handle("IRADS_ZOOM")  -- 1=WIDE(1.4x), 2=MED(10x), 3=NAR(80x)
local MFD_OVERLAY = get_param_handle("MFD_OVERLAY") -- 1 = overlay on, 0 = off
-- EMGY specific
...
local F117_CCRP_PROFILE = get_param_handle("F117_CCRP_PROFILE") -- auto-detected: 0=GBU-31(V)1/B, 1=GBU-31(V)3/B, 2=GBU-32(V)2/B, 3=GBU-12, 4=GBU-27
...
dev:listen_command(Keys.IRADSZoomIn)
dev:listen_command(Keys.IRADSZoomOut)
dev:listen_command(Keys.MFDOverlayToggle)
dev:listen_command(hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_UP)
dev:listen_command(hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_DOWN)
dev:listen_command(hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_LEFT)
dev:listen_command(hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_RIGHT)
dev:listen_command(hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_DEPRESS)
...
HMDMode:set(1)
local RedHUD = 0
local IRADS_EFM_FORWARDING = false
...
-- LEFT DISPLAY ===================	
	...
	IRADS_ZOOM:set(1)			-- 1=WIDE(1.4x), 2=MED(10x), 3=NAR(80x)
	MFD_OVERLAY:set(1)			-- 1 = on, 0 = off
-- EMGY specific
...
-- EMGY specific
	...
	HideFc3Hud()
	ChangeHudColor()
	dispatch_action(nil, 87)   -- activate IRADS (avTVSensor) so MFD shows feed at spawn
	...
	if LD_EMGY_MODE:get() == 0 then
		if (command == keys.LDSK_7) or (command == deviceCommands.LDSK_7) then				-- Upper area cycle. MENU 
			if LD_UPPER:get() == LD_UPPER_INDX then
				...
				SYMB_TOGGLE:set(0) 			-- 1 = on , 0 = off
				SRCH_TRCK_MODE:set(1) 		-- 1 = SEARCH , 2 = TRACK
				dispatch_action(nil,10117) 	-- clear IRADS designation when leaving the pod page
				BOST_TOGGLE:set(0) 			-- 1 = on , 0 = off
				WHOT_TOGGLE:set(0)			-- 1 = on , 0 = off
				IFB_TOGGLE:set(0) 			-- 1 = on , 0 = off
			else
		...
		elseif LD_UPPER:get() == 4 then
			...
			elseif (command == keys.LDSK_4) or (command == deviceCommands.LDSK_4) then	-- toggle Search
				if SRCH_TRCK_MODE:get() == 2 then
					...
					dispatch_action(nil,10117) 	-- clear lock when exiting track
			...
			elseif (command == keys.LDSK_19) or (command == deviceCommands.LDSK_19) then
				if WHOT_TOGGLE:get() == 1 then										-- toggle IR picute ldp
					...
					dispatch_action(nil,10117) 	-- clear designation when changing sensor mode
			...
			if (command == 87) then
				...
				WHOT_TOGGLE:set(0)			-- 1 = on , 0 = off
				IFB_TOGGLE:set(0) 			-- 1 = on , 0 = off
				dispatch_action(nil,10117) 	-- clear designation when pod power state changes
...
-- if fc3 hud mode is changed from a2g, reset all LDP-related values	
	...
	if command == keys.CannonMode then
		if masterMode:get() == 1 then
			if get_param_handle("F117_TYPE"):get() == 1 then
...
-- MFD Overlay Toggle ===================
	if command == Keys.MFDOverlayToggle then
		if MFD_OVERLAY:get() == 1 then
			MFD_OVERLAY:set(0)
		else
			MFD_OVERLAY:set(1)
		end
...
-- IRADS Zoom ===================
	if command == Keys.IRADSZoomIn then
		local z = IRADS_ZOOM:get()
		if z < 3 then
			IRADS_ZOOM:set(z + 1)
		end
		dispatch_action(nil, 103, 1.0)   -- iCommandTVSensorZoomIn / EFM TVSensorZoomIn
	elseif command == Keys.IRADSZoomOut then
		local z = IRADS_ZOOM:get()
		if z > 1 then
			IRADS_ZOOM:set(z - 1)
		end
		dispatch_action(nil, 104, 1.0)   -- iCommandTVSensorZoomOut / EFM TVSensorZoomOut
	elseif command == hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_UP
		or command == hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_DOWN
		or command == hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_LEFT
		or command == hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_RIGHT
		or command == hotas_commands.THROTTLE_DESIGNATOR_CONTROLLER_DEPRESS then
		if not IRADS_EFM_FORWARDING then
			IRADS_EFM_FORWARDING = true
			dispatch_action(nil, command, value)
			IRADS_EFM_FORWARDING = false
		end
	end
